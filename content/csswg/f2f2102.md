...
- css: |
  ...
  caption: |
    Repeated queries for every container?
...
- css: |
  ...
  caption: |
    My proposed change to the `@container` syntax
...
- img: tech
  caption: |
    Respond to nearest ancestor with containment

- title: |
    **1D** Size Containment
  sub: (also from David Baron's proposal)
  md: |
    - **`inline-size`**
    - `block-size`?
    - `width`?
...
- css: |
    aside, main {
      ...
      contain: inline-queries;
    }
  caption: |
    Needs some time in the bike shed...
